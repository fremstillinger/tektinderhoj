...
function MainViewModel(){
    this.windSpeed = ko.observable(0);
    this.windDirection = ko.observable(90);

    this.simulationMode = ko.observable(false);
    this.blinkStatus = ko.observable(false);
    this.wingRotation = ko.observable(0);

    this.livedataConnected = ko.observable(false);

         
}

 setInterval(function(){
            mainViewModel.blinkStatus(!mainViewModel.blinkStatus());
          },1000)

 setInterval(function(){
            mainViewModel.wingRotation((mainViewModel.wingRotation() + mainViewModel.windSpeed()) % 360);
          },1000/25)
...
var connection = new WebSocket(configData.wsAdress);
        connection.onopen = function() {
            mainViewModel.livedataConnected(true);
        };
        ...
        connection.onmessage = function(e) {
                  ...
                  for (var l = 0; l < liveData.length; l++) {
                    
                            var value = liveData[l].value;
                            var readingDate = liveData[l].readingDate;

                           var readingTypeID = liveData[l].readingTypeID;
                           if(readingTypeID == 2){
                                mainViewModel.windSpeed(value);
                           }


                            if(readingTypeID == 7){
                                mainViewModel.windDirection(value);
                           }
